...
function cleanMarkdown(s=''){return s.replace(/\[!\[[^\]]*\]\([^)]*\)\]\([^)]*\)/g,' ').replace(/!\[[^\]]*\]\([^)]*\)/g,' ').replace(/\[([^\]]*)\]\([^)]*\)/g,'$1').replace(/\\([#_*`])/g,'$1').replace(/## Log in or sign up for X[\s\S]*$/i,'').replace(/## See .*?full profile[\s\S]*$/i,'').replace(/\s+/g,' ').replace(/(?:\s+\d+(?:\.\d+)?[KMB]?){2,8}\s*$/i,'').trim();}
function relativeDate(label=''){const now=Date.now();let m=label.match(/^(\d+)(s|m|h|d)$/i);if(m){const mult={s:1000,m:60000,h:3600000,d:86400000}[m[2].toLowerCase()];return new Date(now-Number(m[1])*mult)}m=label.match(/^([A-Z][a-z]{2})\s+(\d{1,2})(?:,\s*(\d{4}))?$/);if(m){const y=m[3]?Number(m[3]):new Date().getUTCFullYear();const d=new Date(`${m[1]} ${m[2]}, ${y} 12:00:00 UTC`);if(!Number.isNaN(d.getTime()))return d}return new Date(0)}
function parseJinaX(text,s){const posts=[];for(const block of text.split(/\n(?=\*\s+)/g)){const re=new RegExp(`\\[([^\\]]+)\\]\\(https:\\/\\/x\\.com\\/${s.handle}\\/status\\/(\\d+)\\)`,'i');const hit=block.match(re);if(!hit)continue;const label=hit[1].trim();if(!/^(?:\d+[smhd]|[A-Z][a-z]{2}\s+\d{1,2}(?:,\s*\d{4})?)$/i.test(label))continue;const body=cleanMarkdown(block.slice((hit.index||0)+hit[0].length));const d=relativeDate(label),ts=d.getTime();if(!body||!ts||!(s.official||s.allPosts||relevantToJuve(body,s)))continue;posts.push({source:s.name,body,date:d.toISOString(),ts,link:`https://x.com/${s.handle}/status/${hit[2]}`})}return posts}
function sleep(ms){return new Promise(r=>setTimeout(r,ms));}
async function fetchJinaX(handle){const c=new AbortController(),t=setTimeout(()=>c.abort(),7000);try{const r=await fetch(`https://r.jina.ai/https://x.com/${handle}`,{cache:'no-store',signal:c.signal,headers:{Accept:'text/plain','User-Agent':'Mozilla/5.0'}});if(!r.ok)throw new Error(String(r.status));const text=await r.text();if(!/status\//i.test(text))throw new Error('empty');return text}finally{clearTimeout(t)}}
const cached=unstable_cache(async h=>fetchJinaX(h),['jump-social-x-coverage-v5'],{revalidate:120});
async function one(s){try{return parseJinaX(await cached(s.handle),s).slice(0,s.allPosts?20:8)}catch{return []}}
