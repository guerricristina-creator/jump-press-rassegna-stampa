...
const X_FEED_BASES=['https://rsshub.yfi.moe','https://rsshub.stsecurity.moe','https://rsshub.edwardcc.com','https://rsshub.isrss.com','https://rsshub.uocat.com'];
const NITTER_BASES=['https://nitter.cf','https://xcancel.com'];
const MONITORED_NAMES=[
 'Tuttosport','La Gazzetta dello Sport','Corriere dello Sport','La Stampa','la Repubblica','Corriere della Sera','ANSA','Sky Sport','Sport Mediaset','Rai Sport','JuventusNews24','TuttoJuve','IlBianconero','Tifo Juventus','GianlucaDiMarzio.com','Alfredo Pedullà','Calciomercato.com','Tuttomercatoweb','Goal Italia','Football Italia','Romeo Agresti','Giovanni Albanese','Guido Vaciago','Nicolò Schira','Gianluca Di Marzio','Fabrizio Romano','Matteo Moretto','Mirko Di Natale','Nicola Balice','Giovanni Guardalà','Paolo Aghemo','Fabiana Della Valle','Filippo Cornacchia','Fabrizio Biasin','Marco Conterio','Andrea Longoni','Daniele Longo'
];
const SOURCES=[
 {name:'Juventus',handle:'juventusfc',official:true},{name:'Tuttosport',handle:'tuttosport'},{name:'La Gazzetta dello Sport',handle:'Gazzetta_it'},
 {name:'Corriere dello Sport',handle:'CorSport'},{name:'La Stampa',handle:'LaStampa'},{name:'Corriere della Sera',handle:'Corriere'},
 {name:'Sky Sport',handle:'SkySport'},{name:'Sport Mediaset',handle:'sportmediaset'},{name:'IlBianconero',handle:'ilbianconerocom'},
 {name:'Gianluca Di Marzio',handle:'DiMarzio'},{name:'Fabrizio Romano',handle:'FabrizioRomano'},{name:'Nicolò Schira',handle:'NicoSchira'},
 {name:'Romeo Agresti',handle:'romeoagresti'},{name:'Giovanni Albanese',handle:'GiovaAlbanese'},{name:'Alfredo Pedullà',handle:'AlfredoPedulla'},
 {name:'Calciomercato.com',handle:'cmdotcom'},{name:'Tuttomercatoweb',handle:'TuttoMercatoWeb'},{name:'Goal Italia',handle:'GoalItalia'},
 {name:'Football Italia',handle:'footballitalia'},{name:'Gianluigi Longari',handle:'Glongari'},
 {name:'Cronache di Spogliatoio',handle:'CronacheTweet'},{name:'Calcio e Finanza',handle:'CalcioFinanza'},{name:'Paolo Ardoino',handle:'paoloardoino',ardoino:true}
];
const DIRECT_NAMES=new Set(SOURCES.map(s=>s.name.toLowerCase()));
const SEARCH_TERMS=['Juventus','Juve',...MONITORED_NAMES.filter(n=>!DIRECT_NAMES.has(n.toLowerCase())).map(n=>`${n} Juventus`)];
const JUVE_TERMS=/(?:\bjuventus\b|\bjuve\b|\bbianconer\w*\b|\bvecchia\s+signora\b|\bcontinassa\b|\bjtc\b|\ballianz\s*stadium\b|@juventusfc\b|juventusfc\b)/i;
...
function relevant(body='',s={}){if(JUVE_TERMS.test(body))return true;return Boolean(s.ardoino&&/\b(zebra|zebre|jay)\b/i.test(body));}
function parseXFeed(xml,s){return [...xml.matchAll(/<item>([\s\S]*?)<\/item>/gi)].map(m=>{const b=m[1],date=xmlText(b,'pubDate')||xmlText(b,'dc:date'),title=cleanHtml(xmlText(b,'title')),desc=cleanHtml(xmlText(b,'description')),enc=cleanHtml(xmlText(b,'content:encoded'));let body=[enc,desc,title].sort((a,b)=>b.length-a.length)[0]||'';body=body.replace(/^RT by @?[^:]+:\s*/i,'').replace(/^RT @?[^:]+:\s*/i,'').trim();const raw=xmlText(b,'link')||xmlText(b,'guid'),status=raw.match(/\/status\/(\d+)/)?.[1]||raw.match(/\/status\/(\d+)/)?.[1];return {source:s.name,body,date,ts:Date.parse(date)||0,link:status?`https://x.com/${s.handle}/status/${status}`:`https://x.com/${s.handle}`};}).filter(p=>p.body&&(s.official||relevant(p.body,s)));}
async function fetchBase(base,handle){const c=new AbortController(),t=setTimeout(()=>c.abort(),2600);try{const r=await fetch(`${base}/twitter/user/${handle}/exclude_replies`,{cache:'no-store',signal:c.signal,redirect:'follow',headers:{'User-Agent':'Mozilla/5.0','Accept':'application/rss+xml, application/xml, text/xml;q=0.9, */*;q=0.5'}});if(!r.ok)throw new Error(String(r.status));const xml=await r.text();if(!/<item>/i.test(xml))throw new Error('empty');return xml;}finally{clearTimeout(t)}}
async function fetchNitterBase(base,handle){const c=new AbortController(),t=setTimeout(()=>c.abort(),3200);try{const r=await fetch(`${base}/${handle}/rss`,{cache:'no-store',signal:c.signal,redirect:'follow',headers:{'User-Agent':'Mozilla/5.0 (iPhone; CPU iPhone OS 18_0 like Mac OS X) AppleWebKit/605.1.15','Accept':'application/rss+xml,application/xml,text/xml;q=0.9,*/*;q=0.5'}});if(!r.ok)throw new Error(String(r.status));const xml=await r.text();if(!/<item>/i.test(xml))throw new Error('empty');return xml;}finally{clearTimeout(t)}}
async function fetchRss(handle){try{return await Promise.any(X_FEED_BASES.map(base=>fetchBase(base,handle)))}catch{throw new Error('rss_unavailable')}}
async function fetchNitter(handle){try{return await Promise.any(NITTER_BASES.map(base=>fetchNitterBase(base,handle)))}catch{throw new Error('nitter_unavailable')}}
...
const rssCache=unstable_cache(async h=>fetchRss(h),['social-api-rss-v8'],{revalidate:180});
const nitterCache=unstable_cache(async h=>fetchNitter(h),['social-api-nitter-v1'],{revalidate:120});
const jinaCache=unstable_cache(async h=>fetchJina(h),['social-api-jina-v8'],{revalidate:300});
async function one(s){
 const [rss,nitter,jina,bing]=await Promise.allSettled([
  rssCache(s.handle).then(xml=>parseXFeed(xml,s)),
  nitterCache(s.handle).then(xml=>parseXFeed(xml,s)),
  jinaCache(s.handle).then(text=>parseJina(text,s)),
  fetchBingX(s)
 ]);
 const all=[rss,nitter,jina,bing].flatMap(r=>r.status==='fulfilled'?r.value:[]);
...
const cachedBuild=unstable_cache(build,['social-feed-build-v8'],{revalidate:90});
export async function GET(){
 try{
  const posts=await cachedBuild();
  return NextResponse.json({posts,generatedAt:new Date().toISOString(),monitored:MONITORED_NAMES.length},{headers:{'Cache-Control':'public, s-maxage=90, stale-while-revalidate=600'}})
 }catch{
